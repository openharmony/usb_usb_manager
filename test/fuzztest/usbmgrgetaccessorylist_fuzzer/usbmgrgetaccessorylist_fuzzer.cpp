...
#include "usbmgrgetaccessorylist_fuzzer.h"
...
const uint32_t code = 0x5A;
const std::u16string USB_INTERFACE_TOKEN = u"OHOS.USB.IUsbServer";

namespace USB {
bool UsbMgrGetAccessoryListFuzzTest(const uint8_t* rawData, size_t size)
{
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::USB::UsbMgrGetAccessoryListFuzzTest(data, size);
    return 0;
}
