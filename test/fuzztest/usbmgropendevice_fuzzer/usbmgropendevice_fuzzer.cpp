...
#include "usbmgropendevice_fuzzer.h"
...
const uint32_t code = 0x03;
const std::u16string USB_INTERFACE_TOKEN = u"OHOS.USB.IUsbServer";
namespace USB {
    bool UsbMgrOpenDeviceFuzzTest(const uint8_t* rawData, size_t size)
    {
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::USB::UsbMgrOpenDeviceFuzzTest(data, size);
    return 0;
}
