...
#include "usbmgrusbcontroltransfer_fuzzer.h"
...
namespace USB {
    bool UsbMgrUsbControlTransferFuzzTest(const uint8_t* data, size_t /* size */)
    {
        ...
        ret = usbSrvClient.UsbControlTransfer(reinterpret_cast<USBDevicePipe &>(data),
            reinterpret_cast<const UsbCtrlTransferParams &>(std::move(data + OFFSET)), buf);
        if (ret == UEC_OK) {
            return false;
        }
        return true;
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::USB::UsbMgrUsbControlTransferFuzzTest(data, size);
    return 0;
}
