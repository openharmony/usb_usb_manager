...
class MockUsbImpl : public IUsbInterface {
    DECLARE_DELAYED_SP_SINGLETON(MockUsbImpl);

public:
    static UsbDevice FindDeviceInfo(std::vector<UsbDevice> &devi);
    ...
    MOCK_METHOD2(GetDeviceDescriptor, int32_t(const UsbDev &dev, std::vector<uint8_t> &decriptor));
