...
HWTEST_F(UsbRequestTest, UsbRequestInitialize002, TestSize.Level1)
{
    ...
    request.SetClientData(cdata);
    uint8_t buffer[BUFFER_SIZE] = "usb request test!";
...
HWTEST_F(UsbRequestTest, UsbrequestQueue002, TestSize.Level1)
{
    ...
    request.SetClientData(cdata);
    uint8_t buffer1[BUFFER_SIZE] = "usb request test!";
    std::vector<uint8_t> bufferdata = {buffer1, buffer1 + 17};
    request.SetReqData(bufferdata);
    ret = request.Initialize(pip, point);
    USB_HILOGI(
...
HWTEST_F(UsbRequestTest, Usbrequestabort004, TestSize.Level1)
{
    ...
    ret = request.Initialize(pip, point);
    EXPECT_TRUE(ret == 0);
    uint8_t buffer[BUFFER_SIZE] = "request abort 004";
    std::vector<uint8_t> databuffer = {buffer, buffer + 17};
    request.SetReqData(databuffer);
    ret = UsbSrvClient.RequestQueue(request);
...
HWTEST_F(UsbRequestTest, Usbrequestabort005, TestSize.Level1)
{
    ...
    ret = UsbSrvClient.ClaimInterface(pip, interface, true);
    EXPECT_TRUE(ret == 0);
    uint8_t tag[TAG_SIZE] = "request abort 005";
    std::vector<uint8_t> cdata = {tag, tag + 17};
    request.SetClientData(cdata);
    ret = request.Initialize(pip, point);
    EXPECT_TRUE(ret == 0);
    uint8_t buffer[BUFFER_SIZE] = "usb request test 005";
...
HWTEST_F(UsbRequestTest, Usbrequestabort006, TestSize.Level1)
{
    ...
    ret = request.Initialize(pip, point);
    EXPECT_TRUE(ret == 0);
    uint8_t buffer[BUFFER_SIZE] = "usb request test 006";
    std::vector<uint8_t> databuffer = {buffer, buffer + 20};
    request.SetReqData(databuffer);
    ret = UsbSrvClient.RequestQueue(request);
...
HWTEST_F(UsbRequestTest, PipeRequestWait004, TestSize.Level1)
{
    ...
    EXPECT_TRUE(ret == 0);
    UsbRequest request;
    uint8_t tag[TAG_SIZE] = "request wait 004";
    std::vector<uint8_t> cdata = {tag, tag + 16};
    request.SetClientData(cdata);
    ret = request.Initialize(pip, point);
    USB_HILOGI(
...
HWTEST_F(UsbRequestTest, PipeRequestWait005, TestSize.Level1)
{
    ...
    EXPECT_TRUE(ret == 0);
    UsbRequest request;
    uint8_t tag[TAG_SIZE] = "request wait 005";
    std::vector<uint8_t> cdata = {tag, tag + 16};
    request.SetClientData(cdata);
    ret = request.Initialize(pip, point);
    EXPECT_TRUE(ret == 0);
    uint8_t buffer[BUFFER_SIZE] = "usb request wait test005";
    std::vector<uint8_t> databuffer = {buffer, buffer + 24};
    request.SetReqData(databuffer);
    ret = UsbSrvClient.RequestQueue(request);
...
HWTEST_F(UsbRequestTest, PipeRequestWait006, TestSize.Level1)
{
    ...
    ret = request.Initialize(pip, point);
    EXPECT_TRUE(ret == 0);
    uint8_t buffer[BUFFER_SIZE] = "usb request wait test006";
    std::vector<uint8_t> databuffer = {buffer, buffer + 24};
    request.SetReqData(databuffer);
    ret = UsbSrvClient.RequestQueue(request);
 ...
 * @tc.type: FUNC
 */
HWTEST_F(UsbRequestTest, UsbHasRight001, TestSize.Level1)
{
 ...
 * @tc.type: FUNC
 */
HWTEST_F(UsbRequestTest, UsbHasRight002, TestSize.Level1)
{
    ...
    USB_HILOGI(MODULE_USB_SERVICE, "UsbRequestTest::UsbHasRight002 RequestRight=%{public}d", result);
    ASSERT_EQ(0, ret);
 ...
 * @tc.type: FUNC
 */
HWTEST_F(UsbRequestTest, UsbHasRight003, TestSize.Level1)
{
    ...
    USB_HILOGI(MODULE_USB_SERVICE, "UsbRequestTest::UsbHasRight003 RemoveRight=%{public}d", result);
    ASSERT_EQ(0, ret);
 ...
 * @tc.type: FUNC
 */
HWTEST_F(UsbRequestTest, UsbHasRight004, TestSize.Level1)
{
    ...
    ASSERT_TRUE(result);
    ret = UsbSrvClient.RequestRight(deviceName);
    USB_HILOGI(MODULE_USB_SERVICE, "UsbRequestTest::UsbHasRight004 RequestRight=%{public}d", result);
    ASSERT_EQ(0, ret);
    result = UsbSrvClient.HasRight(deviceName);
    USB_HILOGI(MODULE_USB_SERVICE, "UsbRequestTest::UsbHasRight004 HasRight=%{public}d", result);
    ASSERT_TRUE(result);
    ret = UsbSrvClient.RemoveRight(deviceName);
    USB_HILOGI(MODULE_USB_SERVICE, "UsbRequestTest::Usbrequestright003 RemoveRight=%{public}d", result);
    ASSERT_EQ(0, ret);
 ...
 * @tc.type: FUNC
 */
HWTEST_F(UsbRequestTest, UsbHasRight005, TestSize.Level1)
{
    USB_HILOGI(MODULE_USB_SERVICE, "Case Start : UsbHasRight005: SetConfig");
    auto &UsbSrvClient = UsbSrvClient::GetInstance();
    std::string deviceName = "device_80";
    bool result = UsbSrvClient.HasRight(deviceName);
    USB_HILOGI(MODULE_USB_SERVICE, "UsbRequestTest::UsbHasRight005 HasRight=%{public}d", result);
    ASSERT_FALSE(result);
    int32_t ret = UsbSrvClient.RequestRight(deviceName);
    USB_HILOGI(MODULE_USB_SERVICE, "UsbRequestTest::UsbHasRight005 RequestRight=%{public}d", result);
    ASSERT_NE(0, ret);
    result = UsbSrvClient.HasRight(deviceName);
    USB_HILOGI(MODULE_USB_SERVICE, "UsbRequestTest::UsbHasRight005 HasRight=%{public}d", result);
    ASSERT_FALSE(result);
    ret = UsbSrvClient.RemoveRight(deviceName);
    USB_HILOGI(MODULE_USB_SERVICE, "UsbRequestTest::UsbHasRight005 RemoveRight=%{public}d", result);
    ASSERT_NE(0, ret);
    USB_HILOGI(MODULE_USB_SERVICE, "Case End : UsbHasRight005: SetConfig");
}
 ...
 * @tc.type: FUNC
 */
HWTEST_F(UsbRequestTest, UsbHasRight006, TestSize.Level1)
{
    USB_HILOGI(MODULE_USB_SERVICE, "Case Start : UsbHasRight006: start");
    auto &UsbSrvClient = UsbSrvClient::GetInstance();
    vector<UsbDevice> delist;
    auto ret = UsbSrvClient.GetDevices(delist);
    EXPECT_TRUE(ret == 0);
    ...
    ASSERT_TRUE(result);
    auto tokenId1 = UsbCommonTest::AllocHapTestWithUserIdDiff();
    UsbCommonTest::SetSelfToken(tokenId1);
    result = UsbSrvClient.HasRight(deviceName);
    USB_HILOGI(MODULE_USB_SERVICE, "UsbRequestTest::UsbHasRight006 HasRight=%{public}d", result);
    ASSERT_FALSE(result);
    ret = UsbSrvClient.RemoveRight(deviceName);
    USB_HILOGI(MODULE_USB_SERVICE, "UsbRequestTest::UsbHasRight006 RemoveRight=%{public}d", ret);
    ASSERT_NE(0, ret);
    auto tokenId2 = UsbCommonTest::AllocHapTestWithIndexDiff();
    UsbCommonTest::SetSelfToken(tokenId2);
    result = UsbSrvClient.HasRight(deviceName);
    USB_HILOGI(MODULE_USB_SERVICE, "UsbRequestTest::UsbHasRight006 HasRight=%{public}d", result);
    ASSERT_FALSE(result);
    ret = UsbSrvClient.RemoveRight(deviceName);
    ...
    ASSERT_EQ(0, ret);
    UsbCommonTest::GrantPermissionSysNative();
    UsbCommonTest::DeleteAllocHapToken(tokenId0);
    UsbCommonTest::DeleteAllocHapToken(tokenId1);
    UsbCommonTest::DeleteAllocHapToken(tokenId2);
    USB_HILOGI(MODULE_USB_SERVICE, "Case End : UsbHasRight006: end");
}
 ...
 * @tc.type: FUNC
 */
HWTEST_F(UsbRequestTest, Usbrequestright001, TestSize.Level1)
{
    ...
    ASSERT_EQ(0, ret);
    bool result = UsbSrvClient.HasRight(deviceName);
    USB_HILOGI(MODULE_USB_SERVICE, "UsbRequestTest::Usbrequestright001 HasRight=%{public}d", result);
    ASSERT_TRUE(result);
    ret = UsbSrvClient.RemoveRight(deviceName);
    USB_HILOGI(MODULE_USB_SERVICE, "UsbRequestTest::Usbrequestright003 RemoveRight=%{public}d", result);
    ASSERT_EQ(0, ret);
...
HWTEST_F(UsbRequestTest, Usbrequestright003, TestSize.Level1)
{
    ...
    ret = UsbSrvClient.RequestRight(deviceName);
    bool result = UsbSrvClient.HasRight(deviceName);
    USB_HILOGI(MODULE_USB_SERVICE, "UsbRequestTest::Usbrequestright003 HasRight=%{public}d", result);
    ASSERT_TRUE(result);
    ret = UsbSrvClient.RequestRight(deviceName);
    USB_HILOGI(MODULE_USB_SERVICE, "UsbRequestTest::Usbrequestright003 RequestRight=%{public}d", result);
    ASSERT_EQ(0, ret);
    result = UsbSrvClient.HasRight(deviceName);
    USB_HILOGI(MODULE_USB_SERVICE, "UsbRequestTest::Usbrequestright003 HasRight=%{public}d", result);
    ASSERT_TRUE(result);
...
{
    USB_HILOGI(MODULE_USB_SERVICE, "Case Start : Usbrequestright004: SetConfig");
    auto &UsbSrvClient = UsbSrvClient::GetInstance();
    std::string deviceName = "device_81";
    bool result = UsbSrvClient.HasRight(deviceName);
    USB_HILOGI(MODULE_USB_SERVICE, "UsbRequestTest::Usbrequestright004 HasRight=%{public}d", result);
    ASSERT_FALSE(result);
    int32_t ret = UsbSrvClient.RequestRight(deviceName);
    USB_HILOGI(MODULE_USB_SERVICE, "UsbRequestTest::Usbrequestright004 RequestRight=%{public}d", result);
    ASSERT_NE(0, ret);
    result = UsbSrvClient.HasRight(deviceName);
    USB_HILOGI(MODULE_USB_SERVICE, "UsbRequestTest::Usbrequestright004 HasRight=%{public}d", result);
    ASSERT_FALSE(result);
    ret = UsbSrvClient.RemoveRight(deviceName);
    USB_HILOGI(MODULE_USB_SERVICE, "UsbRequestTest::Usbrequestright004 RemoveRight=%{public}d", result);
    ASSERT_NE(0, ret);
    USB_HILOGI(MODULE_USB_SERVICE, "Case End : Usbrequestright004: SetConfig");
}
} // namespace Request
} // namespace USB
} // namespace OHOS
